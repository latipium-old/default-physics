...
namespace Com.Latipium.Defaults.Physics {
	internal class WorldBody : RigidBody {
		...
		private readonly Func<Tuple<float, float, float>, Tuple<float, float, float>> PositionFunc;
		private readonly Func<float[], float[]> TransformFunc;

		public override void PreStep(float timestep) {
			Tuple<float, float, float> position = PositionFunc(null);
		...
		public override void PostStep(float timestep) {
			...
			PositionFunc(new Tuple<float, float, float>(position.X, position.Y, position.Z));
		...
		private static Octree CreateOctree(Tuple<float[], int[]> data) {
		...
		internal WorldBody(Tuple<float[], int[]> data, LatipiumObject obj) : base(new TriangleMeshShape(CreateOctree(data))) {
			obj = Instance;
			PositionFunc = Instance.GetFunction<Tuple<float, float, float>, Tuple<float, float, float>>("Position");
